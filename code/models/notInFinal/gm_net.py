"""
Graphical Model Net.
This net output the kernels to be used in the graphical model.
"""
...
class Stage1(nn.Module):
    ...
    def forward(self, x):
        """
        :param x: feature map   4D Tensor   batch size * 128 * 45 * 45
        :return: x              4D Tensor   batch size * 21  * 45 * 45
        """
        x = self.relu(self.stage1_1(x))  # batch size * 512 * 45 * 45
        x = self.stage1_2(x)             # batch size * 21 * 45 * 45
        return x
...
class GMNet(nn.Module):
    ...
    def forward(self, image):
        ...
        # ************* CM stage *************
        stage1 = self.confidence_map_stage1(features)   # batch size * 21 * 45 * 45
