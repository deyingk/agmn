"""
Train unary branch of the proposed Adaptive Graphical Model Network.
The unary branch is actually the CPM baseline.
"""
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=len(dataloaders['train'])) as t:
            for batch_id, data in enumerate(dataloaders['train']):
                ...
                loss = loss_fn(pred_6, gdtt_heatmap)
                loss.backward()
                optimizer.step()

                loss_last = loss_fn(pred_6[:, -1, ...], gdtt_heatmap[:, -1, ...]) # last stage loss
        ...
        with torch.no_grad():
            with tqdm(total=len(dataloaders['valid'])) as t:
                for batch_id, data in enumerate(dataloaders['valid']):
                    ...
                    pred_6 = model(image)  # batch_size x 6 x 21 x 46 x 46
                    loss_last = loss_fn(pred_6[:,-1,...], gdtt_heatmap[:, -1, ...])
    ...
    with torch.no_grad():
        with tqdm(total=len(dataloaders['test'])) as t:
            for batch_id, data in enumerate(dataloaders['test']):
                ...
                pred_6 = model(image)  # batch_size x 7 x 21 x 46 x 46

                # get predicted label
                final_pred = pred_6[:,-1,...]  # last stage
...
if __name__ == '__main__':
    ...
    model_dir = os.path.join('../checkpoint',config['data_loader']['name'], config['model_name'])
