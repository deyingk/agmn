...
def train_and_evaluate(model, dataloaders, optimizer, loss_fn, loss_coef, model_dir, device, gdtt_all_label_dict, num_epochs=100, restore_file_path=None):
...
if __name__ == '__main__':
    ...
    loss_coef = config.get("loss_coef",[1.0,0.0,0.0])

    # train
    logger.info('Training the model for loss_coef: '+str(loss_coef)+'\n')
    train_and_evaluate(model, dataloaders, optimizer, loss_fn, loss_coef, model_dir, device, gdtt_all_label_dict, num_epochs=num_epochs, restore_file_path=None)
