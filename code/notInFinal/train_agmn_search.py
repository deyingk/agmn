"""
This script is used to train adaptive graphical model network (AGMN).
...
def train_and_evaluate(model, dataloaders, optimizer, loss_fn, model_dir, device, gdtt_all_label_dict, num_epochs=100, restore_file_path=None, loss_coef=None):
...
if __name__ == '__main__':
    ...
    loss_coef_list = [[1, coef_2*0.02, coef_3*0.02] for coef_2 in range(6) for coef_3 in range(6)]

    for loss_coef in loss_coef_list:

        # make experiment directory
        now = datetime.now()
        date_time = now.strftime("%m-%d-%H-%M-%S")
        model_dir = os.path.join('../checkpoint',config['data_loader']['name'], config['model_name'],'JointlyTrained', date_time)
        ...
        # train
        logger.info('Training the model for loss_coef: '+str(loss_coef)+'\n')
        train_and_evaluate(model, dataloaders, optimizer, loss_fn, model_dir, device, gdtt_all_label_dict, num_epochs=num_epochs, restore_file_path=None, loss_coef=loss_coef)
        ...
        # perform testing
        logger.info('Testing the model for loss_coef: '+str(loss_coef)+'\n')
