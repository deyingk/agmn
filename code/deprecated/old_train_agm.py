"""
This script is used to train adaptive graphical model.
"""
    ...
    for epoch in range(num_epochs):
        ...
        with tqdm(total=len(dataloaders['train'])) as t:
            for batch_id, data in enumerate(dataloaders['train']):
                ...
                gdtt_heatmap = torch.stack([gdtt_heatmap]*7, dim=1).to(device)

                gdtt_gm_kernels = data['gm_kernels']
                gdtt_gm_kernels = torch.stack([gdtt_gm_kernels]*3, dim=1).to(device)

                optimizer.zero_grad()
                gm_kernels_3, pred_7 = model(image)  
                # gm_kernels_3: batch_size x 3 x 40 x 45 x 45
                # pred_7: batch_size x 7 x 40 x 46 x 46
                # print('gm_kernels_3 #############################################')
                # print(gm_kernels_3[:,-1,...])
                
                loss = loss_fn(pred_7, gdtt_heatmap) + loss_fn(gm_kernels_3, gdtt_gm_kernels)
                loss.backward()
                optimizer.step()

                loss_last = loss_fn(pred_7[:,-1,...], gdtt_heatmap[:,-1,...])
