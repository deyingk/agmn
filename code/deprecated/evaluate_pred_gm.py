...
def evaluate_pred_gm(cpm_model, gm_model, dataloaders, model_dir, device, gdtt_all_label_dict):
    ...
    with torch.no_grad():
        with tqdm(total=len(dataloaders['valid'])) as t:
            for batch_id, data in enumerate(dataloaders['valid']):
                ...
                pred_6 = cpm_model(image)  # batch_size x 6 x 21 x 46 x 46

                gm_kernels = gm_model(image)  # batch_size x 6 x 40 x 46 x 46
                # get predicted label
                ...
                final_pred = tree_model(pred_last, gm_kernels[:,-1,...].relu(), bias)

                final_pred = final_pred.cpu()
                # print(final_pred.shape)
                for image_i in range(len(image_names)):
                    # print(final_pred[image_i].shape)
                ...
                for image_i in range(len(image_names)):
                    ...
                    cpm_pred_label_dict[image_names[image_i]] = {}
                    cpm_pred_label_dict[image_names[image_i]]['pred_label'] =  predicted_label
                    cpm_pred_label_dict[image_names[image_i]]['resol'] =  float(original_image_sizes[image_i][0])


                t.update()
        confirm_dir(os.path.join(model_dir,'valid_gm_pred_labels'))
        with open(os.path.join(model_dir,'valid_gm_pred_labels','pred_gm_labels.json'),'w') as f:
            json.dump(pred_label_dict, f)

        confirm_dir(os.path.join(model_dir,'valid_cpm_pred_labels'))
        with open(os.path.join(model_dir,'valid_cpm_pred_labels','pred_cpm_labels.json'),'w') as f:
            json.dump(cpm_pred_label_dict, f)
        ...
        for image_name in pred_label_dict.keys():
            ...
            cur_pck = cur_pck + pck
        val_acc = cur_pck/len(pred_label_dict)
        logger.info('predicted GM + OpenPoseHand ' +str(val_acc))

        cur_pck = 0.0
        for image_name in cpm_pred_label_dict.keys():
            pred_label = cpm_pred_label_dict[image_name]['pred_label']
            pred_resol = cpm_pred_label_dict[image_name]['resol']
            gdtt_label = gdtt_all_label_dict[image_name]
            pck = utils.PCK(pred_label, gdtt_label, pred_resol, sigma=0.04)
            cur_pck = cur_pck + pck
        val_acc = cur_pck/len(cpm_pred_label_dict)
        logger.info('Pure OpenPoseHand ' +str(val_acc))
    ...
    with torch.no_grad():
        with tqdm(total=len(dataloaders['test'])) as t:
            for batch_id, data in enumerate(dataloaders['test']):
                ...
                t.update()
        confirm_dir(os.path.join(model_dir,'test_gm_pred_labels'))
        with open(os.path.join(model_dir,'test_gm_pred_labels','pred_gm_labels.json'),'w') as f:
            json.dump(pred_label_dict, f)

        confirm_dir(os.path.join(model_dir,'test_cpm_pred_labels'))
        with open(os.path.join(model_dir,'test_cpm_pred_labels','pred_cpm_labels.json'),'w') as f:
            json.dump(cpm_pred_label_dict, f)
        ...
        for image_name in pred_label_dict.keys():
            ...
            pck = utils.PCK(pred_label, gdtt_label, pred_resol, sigma=0.04)
            cur_pck = cur_pck + pck
        test_acc = cur_pck/len(pred_label_dict)
        logger.info('predicted GM + OpenPoseHand , test dataset ' +str(test_acc))
        ...
        for image_name in cpm_pred_label_dict.keys():
            ...
            pck = utils.PCK(pred_label, gdtt_label, pred_resol, sigma=0.04)
            cur_pck = cur_pck + pck
        test_acc = cur_pck/len(cpm_pred_label_dict)
        logger.info('Pure OpenPoseHand, test dataset ' +str(test_acc))
...
if __name__ == '__main__':
    ...
    # get model
    cpm_model_class = getattr(models,config['cpm_model_name'])
    cpm_model = cpm_model_class(21)
    cpm_model = cpm_model.to(device)
    cpm_model = torch.nn.DataParallel(cpm_model)

    gm_model_class = getattr(models,config['gm_model_name'])
    gm_model = gm_model_class(40)
    gm_model = gm_model.to(device)
    gm_model = torch.nn.DataParallel(gm_model)




    #### load trained model
    cpm_model_dir = config['cpm_model_dir']
    print(cpm_model_dir)
    utils.load_checkpoint(os.path.join(cpm_model_dir, 'best.tar'), cpm_model)

    gm_model_dir = config['gm_model_dir']
    utils.load_checkpoint(os.path.join(gm_model_dir, 'best.tar'), gm_model)
    
    evaluate_pred_gm(cpm_model, gm_model, dataloaders, model_dir, device, gdtt_all_label_dict)
